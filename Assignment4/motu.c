...
//function to non-blocking wait/down the semaphore
void semaphore_wait_nonblock(struct my_semaphore *semaphore) {
  while(pthread_mutex_trylock(semaphore->mutex)){  //checking if locked
       semaphore->value--;  //decreasing value of the semaphore
   	pthread_mutex_unlock(semaphore->mutex); //unlocking the mutex
   } 
  if (semaphore->value < 0) {
...
//function to non-blocking signal/up the semaphore
void semaphore_signal_nonblock(struct my_semaphore *semaphore) {
   while(pthread_mutex_trylock(semaphore->mutex)){  //checking if locked
        semaphore->value++; //increasing value of the semaphore
   	pthread_mutex_unlock(semaphore->mutex); //unlocking the mutex
   } 
  if (semaphore->value <= 0) { 
    semaphore->wakeups++; //incrementing the number of times the semaphore is used
    pthread_cond_broadcast(semaphore->cond); //signaling the condition variable for all threads
  }
   pthread_mutex_unlock(semaphore->mutex);  //unlocking the mutex
}
...
void check_left_right(int phnum){ 
    if (state[phnum] == HUNGRY && state[LEFT] != EATING && state[RIGHT] != EATING){ 
        // state that eating 
        state[phnum] = EATING; 
        semaphore_wait_nonblock(saucebowls[0]); //waiting for sauce bwol 1 to get free
        semaphore_wait_nonblock(saucebowls[1]); //waiting for sauce bwol 2 to get free
        sleep(2); //wait time to make output readable on console
        printf("Philosopher %d eats using forks %d and %d\n", phnum + 1, LEFT + 1, phnum + 1); 
        semaphore_signal_nonblock(forks[phnum]); 
        semaphore_signal_nonblock(saucebowls[0]); //signal that sauce bowl 1 is free
        semaphore_signal_nonblock(saucebowls[1]); //signal that sauce bowl 2 is free
    } 
} 
  
// function to get the forks on left and right by checking the condition using above function
void get_forks(int phnum){ 
    semaphore_wait_nonblock(semaphore); //waiting till the last philospher has returned the forks
    ...
    semaphore_signal_nonblock(semaphore); //signaling semaphore so that philosopher can return the forks
    semaphore_wait_nonblock(forks[phnum]); //waiting for the fork to get free by philosopher
} 
  
//function to return the forks after eating so that left and right philosophers can eat
void return_forks(int phnum){ 
    semaphore_wait_nonblock(semaphore); //waiting for philosopher to finish eating 
    ...
    semaphore_signal_nonblock(semaphore); //signaling so that next philosopher can eat
} 
